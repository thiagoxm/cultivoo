...
// Custo médio — TODAS as entradas (para valor em estoque)
...
// Custo médio — apenas últimos 12 meses (para exibição do preço recente)
...
// Validade mais crítica entre os lotes
...
export default function Estoque() {
  ...
  const [filtroAlerta, setFiltroAlerta] = useState(null) // 'validade' | 'minimo' | null
  ...
  const produtosEnriquecidos = useMemo(() => {
    return produtos.map(p => {
      ...
      const custoMedioTotal = calcularCustoMedioTotal(movs)   // para valor em estoque
      const custoMedio12m = calcularCustoMedio12m(movs)       // para exibição de preço recente
      ...
      const temPagPendente = movs.some(m => m.tipoMov === 'entrada' && m.statusPagamento === 'pendente')
  ...
  const dashboards = useMemo(() => {
    const comValidadeAlerta = produtosEnriquecidos.filter(p =>
      p.validade?.tipo === 'vencido' || p.validade?.tipo === 'alerta'
    ).length
    const comMinimoAlerta = produtosEnriquecidos.filter(p => p.abaixoMinimo).length

    // Valor em estoque usa custo médio de TODAS as entradas
    const valorTotalEstoque = produtosEnriquecidos.reduce((a, p) =>
      a + (p.saldo * p.custoMedioTotal), 0
    )

    const movsPendentes = movimentacoes.filter(m =>
      m.tipoMov === 'entrada' && m.statusPagamento === 'pendente'
    )
    ...
    return { comValidadeAlerta, comMinimoAlerta, valorTotalEstoque, pendValor, pendQtd, ultimaMov }
  ...
  const agrupado = useMemo(() => {
    ...
    if (filtroAlerta === 'validade') {
      base = base.filter(p => p.validade?.tipo === 'vencido' || p.validade?.tipo === 'alerta')
    }
    if (filtroAlerta === 'minimo') {
      base = base.filter(p => p.abaixoMinimo)
    }
    if (filtroPagPendente) {
      base = base.filter(p => p.temPagPendente)
    }
      ...
      .map(([propId, grupo]) => ({
        ...
        tipos: TIPOS_INSUMOS
          ...
          .map(t => {
            ...
            // Valor em estoque usa custo médio total
  ...
  // Saída é transferência ou venda — campos simplificados
  ...
  async function salvarMov(e) {
    ...
    // Validações de vínculo — só para aplicação/consumo
    ...
    if (formMov.tipoMov === 'entrada') {
      ...
      await addDoc(collection(db, 'financeiro'), {
        ...
        origemEstoque: true, produtoId: formMov.produtoId,
    ...
    if (formMov.tipoMov === 'saida' && formMov.tipoSaida === 'transferencia') {
      ...
      await addDoc(collection(db, 'movimentacoesInsumos'), {
        ...
        dataVencimentoPagamento: '', notaRef: '', dataValidade: '',
  ...
  return (
    <div className="space-y-5 pb-24">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        {/* Alertas — clicável, 2 indicadores */}
        <div className="bg-white rounded-xl p-3 shadow-sm border border-gray-100 space-y-2">
          ...
          <button type="button"
            ...
            <span>{dashboards.comValidadeAlerta} vencimento{dashboards.comValidadeAlerta !== 1 ? 's' : ''}</span>
          </button>
          <button type="button"
            ...
            <span>{dashboards.comMinimoAlerta} abaixo do mínimo</span>
          </button>
          {(filtroAlerta) && (
            <button onClick={() => setFiltroAlerta(null)}
              className="text-xs text-gray-400 hover:text-red-400 underline w-full text-left">
              Limpar filtro
            </button>
          )}
        </div>

        {/* Valor em estoque — custo médio total, sem texto de período */}
        ...
        <div className={`bg-white rounded-xl p-3 shadow-sm border cursor-pointer transition-colors ${
          ...
          <p className={`text-xs mt-0.5 ${dashboards.pendQtd > 0 ? 'text-yellow-600' : 'text-gray-400'}`}>
            {dashboards.pendQtd} pagamento{dashboards.pendQtd !== 1 ? 's' : ''} pendente{dashboards.pendQtd !== 1 ? 's' : ''}
          </p>
          {filtroPagPendente && (
            <p className="text-xs text-yellow-500 mt-1 underline">Clique para limpar filtro</p>
          )}
        </div>
              ...
              {propGrupo.tipos.map(tipoGrupo => {
                ...
                return (
                  <div key={tipoGrupo.value} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    {/* ── Produtos — zebra corrigida ── */}
                    {expandido && (
                      <div className="border-t border-gray-100 divide-y divide-gray-100">
                        {tipoGrupo.itens.map((produto, idx) => {
                          // Zebra: baseado no índice dentro deste tipo
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center gap-3 mt-0.5">
                                    ...
                                    {produto.custoMedio12m !== null ? (
                                      <p className="text-xs text-gray-400">
                                        R$ {formatarMoeda(produto.custoMedio12m)}/{produto.unidade} (preço 12m)
                                      </p>
                                    ) : (
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl max-h-[92vh] overflow-y-auto">
            ...
            <form onSubmit={salvarMov} className="p-5 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {tiposSaidaDisponiveis.map(ts => (
                      <button key={ts.value} type="button"
                        onClick={() => setFormMov(f => ({ ...f, tipoSaida: ts.value, safraId: '', lavouraIds: [], patrimonioId: '' }))}
              ...
              {/* ── CAMPOS SAÍDA SIMPLES (transferência / venda) ── */}
                  ...
                  {formMov.tipoSaida === 'transferencia' && (
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-1">
                        O insumo será adicionado ao estoque da propriedade de destino sem impacto financeiro.
                      </p>
                    </div>
                  )}
              ...
              {/* ── CAMPOS SAÍDA APLICAÇÃO/CONSUMO ── */}
              {formMov.tipoMov === 'saida' && !saidaSimples && (
                <>
                  {/* Safra */}
                  ...
                  {vinculos.patrimonio !== 'oculto' && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        Equipamento{' '}
                        {vinculos.patrimonio === 'obrigatorio'
                          ? <span className="text-red-500">*</span>
                          : <span className="text-gray-400 font-normal text-xs">(opcional)</span>}
                      </label>
                    ...
                    <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                      ...
                      {prod.custoMedio12m !== null ? (
                        <p className="text-lg font-bold text-gray-700 mt-0.5">
                          R$ {formatarMoeda(prod.custoMedio12m)}
                        ...
                        <div key={m.id || idx}
                          ...
                          <div className="flex items-start gap-2 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              {m.dataValidade && (
                                <p className="text-xs text-gray-400">Validade: {formatarData(m.dataValidade)}</p>
                              )}
                            </div>
                          </div>
