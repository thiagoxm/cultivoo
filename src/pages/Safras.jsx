...
export default function Safras() {
  ...
  async function carregar() {
    ...
    // Dados compartilhados com permissão 'safras'
    const idsComSafras = (propriedadesCompartilhadas || [])
      .filter(c => c.permissoes.includes('safras'))
      .map(c => c.propriedadeId)

    let safsCompartilhadas = []
    let propsCompartilhadas = []
    let lavsCompartilhadas = []
    for (const propId of idsComSafras) {
      ...
      safsCompartilhadas.push(...ss.docs.map(d => ({ id: d.id, ...d.data(), _compartilhada: true })))
    ...
    setLista([...minhasSafras, ...safsCompartilhadas])
  ...
  async function salvar(e) {
    ...
    const propObj = propriedades.find(p => p.id === payload.propriedadeId)
    if (propObj?._compartilhada) payload.uid = propObj.uid
    if (editando) {
    ...
    <div className="space-y-5 pb-24">
      ...
      {listaFiltrada.length === 0 && (
        <div className="bg-white rounded-xl p-10 text-center text-gray-400 shadow-sm border border-gray-100">
          ...
          <p className="text-sm">
            {aba === 'atuais' ? 'Nenhuma safra atual cadastrada.' : 'Nenhuma safra passada encontrada.'}
          </p>
        </div>
      )}
          ...
          return (
            <div key={s.id}
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm text-gray-500 mt-0.5">{s.cultura} · {s.propriedadeNome}</p>
                {qtdeLavouras > 0 && (
                ...
                <div className="flex items-center gap-3 mt-1 flex-wrap">
                  {(s.dataInicio || s.dataPlantio) && (
                    <p className="text-xs text-gray-400">
                      🌱 Início: <span className="font-medium text-gray-600">{formatarData(s.dataInicio || s.dataPlantio)}</span>
                    </p>
                  )}
                  {(s.dataTermino || s.dataColheitaPrev) && (
                    <p className="text-xs text-gray-400">
                      🌾 Término: <span className="font-medium text-gray-600">{formatarData(s.dataTermino || s.dataColheitaPrev)}</span>
                    </p>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-1 flex-shrink-0">
                {!s._compartilhada && (
                  <>
                    ...
                    <button onClick={() => excluir(s.id, s.nome)} className="text-gray-300 hover:text-red-500 p-1 transition-colors">
                      <Trash2 size={15} />
                    </button>
                  </>
                )}
        ...
        {fabAberto && (
          <div className="flex flex-col items-end gap-2 mb-1">
            <div className="flex items-center gap-2">
              <span className="bg-white text-gray-600 text-xs px-3 py-1.5 rounded-full shadow border border-gray-200 whitespace-nowrap">Nova safra</span>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl max-h-[92vh] overflow-y-auto">
            <div className="p-5 border-b border-gray-100 sticky top-0 bg-white z-10 flex items-center justify-between">
              <h2 className="font-bold text-gray-800">{editando ? 'Editar safra' : 'Nova safra'}</h2>
              <button onClick={() => { setModal(false); setEditando(null) }} className="text-gray-400 hover:text-gray-600">
                <X size={18} />
              </button>
            </div>
            <form onSubmit={salvar} className="p-5 space-y-4">
              ...
              <div>
                ...
                <select value={form.cultura}
                  onChange={e => {
                    ...
                    setForm(f => ({ ...f, cultura: nomeCultura, unidade: config?.unidadePadrao || 'sc', grupo: config?.grupo || 'outros' }))
                  }}
                  className="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" required>
                  <option value="">Selecione...</option>
                  {culturas.map(c => {
                    const config = getCultura(c)
                    return <option key={c} value={c}>{config?.icone ? `${config.icone} ${c}` : c}</option>
                  })}
                </select>
              </div>
              ...
              {form.cultura && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Unidade de medida da colheita</label>
                  <select value={form.unidade} onChange={e => setForm(f => ({ ...f, unidade: e.target.value }))}
                    className="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500">
                    {UNIDADES.map(u => <option key={u.value} value={u.value}>{u.label}</option>)}
                  </select>
                  <p className="text-xs text-gray-400 mt-1">Sugerido para {form.cultura}. Todas as colheitas desta safra usarão esta unidade.</p>
                </div>
              )}
                    ...
                    {form.lavouraIds.length > 0 && (
                      <span className="text-xs text-gray-500">
                        ...
                        <span className="font-semibold text-gray-700">
                          {lavouras.filter(l => form.lavouraIds.includes(l.id)).reduce((a, l) => a + (Number(l.areaHa) || 0), 0).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ha
                        </span>
                      </span>
                    )}
                  </div>
                  <div className="space-y-2 bg-gray-50 rounded-xl p-3">
                    {lavourasDaPropriedade.map(l => (
                      <label key={l.id} className="flex items-center gap-2 text-sm cursor-pointer">
                        <input type="checkbox" checked={form.lavouraIds.includes(l.id)} onChange={() => toggleLavoura(l.id)} className="accent-green-600 w-4 h-4" />
                        <span className="text-gray-700">{l.nome}</span>
                        <span className="text-gray-400 text-xs">({Number(l.areaHa || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ha)</span>
                      </label>
                    ))}
                  </div>
                </div>
