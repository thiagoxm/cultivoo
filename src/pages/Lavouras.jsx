...
function DesenhoPoligono({ onCreated, poligonoInicial, centroProp }) {
  ...
  const poligonoBackupRef = useRef(null)
  ...
  return (
    <>
      <div className="leaflet-top leaflet-right" style={{ marginTop: '10px', marginRight: '10px' }}>
        <div className="leaflet-control flex flex-col gap-1.5">
          {estado === 'normal' && (
            <>
              {centroProp && (
                <button onClick={centralizar} className={`${btnBase} hover:bg-blue-50 hover:border-blue-400 text-gray-700`} style={{ zIndex: 1000 }}>
                  🎯 Centralizar
                </button>
              )}
              <button onClick={iniciarDesenho} className={`${btnBase} hover:bg-green-50 hover:border-green-400 text-gray-700`} style={{ zIndex: 1000 }}>
                ✏️ {temPoligono ? 'Redesenhar área' : 'Desenhar área'}
              </button>
              {temPoligono && (
                <button onClick={iniciarEdicao} className={`${btnBase} hover:bg-yellow-50 hover:border-yellow-400 text-gray-700`} style={{ zIndex: 1000 }}>
                  🔧 Editar área
                </button>
              )}
              {temPoligono && (
                <button onClick={() => setConfirmarApagar(true)} className={`${btnBase} hover:bg-red-50 hover:border-red-400 text-gray-500`} style={{ zIndex: 1000 }}>
                  🗑️ Apagar área
                </button>
              )}
            </>
          ...
          {estado === 'desenhando' && (
            <>
              ...
              <button onClick={cancelarDesenho} className={`${btnBase} hover:bg-red-50 hover:border-red-400 text-red-500`} style={{ zIndex: 1000 }}>
          ...
          {estado === 'editando' && (
            <>
              ...
              <button onClick={salvarEdicao} className={`${btnBase} hover:bg-green-50 hover:border-green-400 text-green-700`} style={{ zIndex: 1000 }}>
                ✓ Salvar edição
              </button>
              <button onClick={cancelarEdicao} className={`${btnBase} hover:bg-red-50 hover:border-red-400 text-red-500`} style={{ zIndex: 1000 }}>
                ✕ Descartar edição
              </button>
            </>
          )}
      ...
      {confirmarApagar && (
        <div style={{ position: 'absolute', inset: 0, zIndex: 2000, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
          <div style={{ background: '#fff', borderRadius: '16px', padding: '24px', maxWidth: '320px', width: '100%', boxShadow: '0 10px 40px rgba(0,0,0,0.2)' }}>
            <h3 style={{ fontWeight: 700, color: '#1f2937', marginBottom: '8px', fontSize: '15px' }}>Apagar área?</h3>
            <p style={{ fontSize: '13px', color: '#6b7280', marginBottom: '4px' }}>O polígono desta lavoura será removido.</p>
            <p style={{ fontSize: '12px', color: '#ef4444', marginBottom: '20px' }}>Esta ação não poderá ser desfeita.</p>
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setConfirmarApagar(false)} style={{ flex: 1, padding: '8px', borderRadius: '10px', border: '1px solid #d1d5db', background: '#fff', color: '#6b7280', fontSize: '13px', cursor: 'pointer' }}>Cancelar</button>
              <button onClick={confirmarApagarPoligono} style={{ flex: 1, padding: '8px', borderRadius: '10px', border: 'none', background: '#dc2626', color: '#fff', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>Apagar</button>
            </div>
          </div>
        </div>
      )}
...
export default function Lavouras() {
  ...
  async function carregar() {
    ...
    const minhasLavouras = lavSnap.docs.map(d => ({ id: d.id, ...d.data() }))
    const minhasProps = propSnap.docs.map(d => ({ id: d.id, ...d.data() }))

    // Dados compartilhados com permissão 'lavouras'
    const idsComLavoura = (propriedadesCompartilhadas || [])
      .filter(c => c.permissoes.includes('lavouras'))
      .map(c => c.propriedadeId)

    let lavsCompartilhadas = []
    let propsCompartilhadas = []
    for (const propId of idsComLavoura) {
    ...
    setLista([...minhasLavouras, ...lavsCompartilhadas])
  ...
  function onDesenho(e) {
    if (!e.layer) { setPoligono(null); setAreaHa(0); return }
  ...
  async function salvar(e) {
    ...
    if (form.modoArea === 'mapa' && !poligono) return alert('Desenhe a área da lavoura no mapa.')
    if (form.modoArea === 'manual' && !form.areaManual) return alert('Informe a área da lavoura.')
    setLoading(true)
  ...
  function MiniMapaGrupo({ propId, grupo }) {
    ...
    function CardSituacao({ label, lavouras, corBadge }) {
      ...
      return (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm flex flex-col flex-1 min-w-0" style={{ minWidth: '120px' }}>
          <div className="flex items-center justify-center px-2 pt-2.5 pb-1.5">
            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${corBadge}`}>{label}</span>
          </div>
      ...
      return (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm px-4 py-2.5 flex items-center justify-center gap-6 w-full">
          <div className="text-center">
            ...
            <p className="text-xl font-bold text-gray-800 leading-tight">{grupo.lavouras.length}</p>
          </div>
    ...
    function MapaConteudo({ altura = '100%' }) {
      return (
        <MapContainer center={centro} zoom={14} style={{ height: altura, width: '100%' }} zoomControl={true} dragging={true}>
          ...
          {grupo.lavouras.map(l =>
            l.poligono?.length > 2 && (
              <Polygon key={l.id} positions={l.poligono.map(p => [p.lat, p.lng])}
                pathOptions={{ color: corLavoura(l.id), fillColor: corLavoura(l.id), fillOpacity: 0.35, weight: 2 }}>
    ...
    return (
      <>
          ...
          <div className="flex-1 relative rounded-xl overflow-hidden border border-gray-100 shadow-sm">
            {temPoligonos ? (
              <>
                ...
                <button onClick={() => setMiniMapaFullscreen(propId)}
              ...
              <div className="h-full flex items-center justify-center bg-gray-50 rounded-xl text-gray-300 text-sm">Sem polígonos cadastrados</div>
            )}
          </div>
        </div>
        ...
        <div className="md:hidden space-y-2">
          <CardTotal />
          <button className="w-full flex items-center justify-between px-4 py-2.5 bg-white rounded-xl border border-gray-100 shadow-sm text-xs text-gray-600"
            ...
            <div className="flex gap-2">
              {ativas.length > 0 && (
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm flex flex-col flex-1 min-w-0 py-2.5 px-2">
                  <div className="flex justify-center mb-1.5"><span className="text-xs px-2 py-0.5 rounded-full font-medium bg-green-100 text-green-700">Ativas</span></div>
              ...
              {emPreparo.length > 0 && (
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm flex flex-col flex-1 min-w-0 py-2.5 px-2">
                  <div className="flex justify-center mb-1.5"><span className="text-xs px-2 py-0.5 rounded-full font-medium bg-yellow-100 text-yellow-700">Em preparo</span></div>
                  <div className="h-px bg-gray-100 mx-1 mb-1.5" />
                  <div className="text-center">
                    <p className="text-xs text-gray-400 leading-tight">Qtde. de Lavoura(s)</p>
              ...
              {ociosas.length > 0 && (
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm flex flex-col flex-1 min-w-0 py-2.5 px-2">
                  <div className="flex justify-center mb-1.5"><span className="text-xs px-2 py-0.5 rounded-full font-medium bg-gray-100 text-gray-500">Ociosas</span></div>
                  <div className="h-px bg-gray-100 mx-1 mb-1.5" />
                  <div className="text-center">
                    <p className="text-xs text-gray-400 leading-tight">Qtde. de Lavoura(s)</p>
  ...
  const grupoFullscreen = miniMapaFullscreen ? agrupado.find(([id]) => id === miniMapaFullscreen) : null
        ...
        <div key={propId} className="space-y-2">
          ...
          <div className="space-y-1.5">
            {grupo.lavouras.map(l => {
              ...
              return (
                <div key={l.id} className="bg-white rounded-lg px-4 py-3 shadow-sm border border-gray-100 flex items-center justify-between gap-2">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${corStatus(l.status)}`}>{l.status}</span>
                        <span className="text-xs text-gray-400">{areaDisp} {labelMedida(grupo.medida)}</span>
                      </div>
                    </div>
                  </div>
              ...
              <form onSubmit={salvar} className="p-5 space-y-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2 mb-3">
                    {[{ val: 'mapa', label: 'Selecionar no mapa' }, { val: 'manual', label: 'Inserir manualmente' }].map(op => (
                      <button key={op.val} type="button"
                        onClick={() => { setForm(f => ({ ...f, modoArea: op.val })); if (op.val === 'mapa') { setPoligono(null); setAreaHa(0) } }}
                        className={`flex-1 py-2 rounded-xl text-sm border-2 transition-colors ${form.modoArea === op.val ? 'border-green-600 bg-green-50 text-green-700 font-medium' : 'border-gray-200 text-gray-500 hover:border-gray-300'}`}>
                        {op.label}
                      </button>
                    ))}
                  </div>
                      ...
                      {!form.propriedadeId ? (
                        ...
                        <>
                          <div className="flex items-center justify-between">
                            <p className="text-xs text-gray-500">{poligono ? '✓ Área desenhada — use tela cheia para editar' : 'Abra tela cheia para desenhar a área'}</p>
                            <div className="flex items-center gap-1">
                              {[{ val: 'normal', label: 'Mapa' }, { val: 'satelite', label: 'Satélite' }].map(c => (
                                <button key={c.val} type="button" onClick={() => setForm(f => ({ ...f, camadaMapa: c.val }))}
                                  className={`text-xs px-2.5 py-1 rounded-lg border transition-colors ${form.camadaMapa === c.val ? 'bg-green-700 text-white border-green-700' : 'border-gray-200 text-gray-500 hover:border-gray-300'}`}>{c.label}</button>
                              ))}
                          ...
                          <div className="h-56 rounded-xl overflow-hidden border border-gray-200">
                            <MapContainer key={`preview-${form.propriedadeId}`}
                              center={centroProp ? [centroProp.lat, centroProp.lng] : [-15.7801, -47.9292]}
                              zoom={centroProp ? 14 : 5} style={{ height: '100%', width: '100%' }} zoomControl={false}>
                              ...
                              {poligono?.length > 2 && <Polygon positions={poligono.map(p => [p.lat, p.lng])} pathOptions={{ color: '#16a34a', fillColor: '#16a34a', fillOpacity: 0.3 }} />}
                            </MapContainer>
                          </div>
                          {!poligono ? (
                            <button type="button" onClick={() => { setMapaKey(k => k + 1); setMapaFullscreen(true) }}
                              ...
                              <Maximize2 size={15} /> Abrir tela cheia para desenhar a área
                            </button>
                          ) : (
                            <button type="button" onClick={() => { setMapaKey(k => k + 1); setMapaFullscreen(true) }}
                              ...
                              <Maximize2 size={13} /> Editar área no mapa
                            </button>
                          )}
                          {areaHa > 0 && (
        ...
        <div className="fixed inset-0 z-[70] bg-black flex flex-col">
          <div className="flex items-center justify-between px-4 py-3 bg-gray-900">
            ...
            <div className="flex items-center gap-2">
              {[{ val: 'normal', label: 'Mapa' }, { val: 'satelite', label: 'Satélite' }].map(c => (
                <button key={c.val} type="button" onClick={() => setForm(f => ({ ...f, camadaMapa: c.val }))}
                  className={`text-xs px-2.5 py-1.5 rounded-lg border transition-colors ${form.camadaMapa === c.val ? 'bg-green-700 text-white border-green-700' : 'border-gray-600 text-gray-300 hover:border-green-500'}`}>{c.label}</button>
              ))}
          ...
          <div className="flex-1">
            <MapContainer key={`fullscreen-${form.propriedadeId}-${mapaKey}`}
              center={centroProp ? [centroProp.lat, centroProp.lng] : [-15.7801, -47.9292]}
              zoom={centroProp ? 14 : 5} style={{ height: '100%', width: '100%' }}>
              <TileLayers camada={form.camadaMapa} />
              {centroProp && <CentralizarMapa centro={centroProp} zoom={14} />}
              {centroProp && <Marker position={[centroProp.lat, centroProp.lng]} icon={iconeProp} />}
