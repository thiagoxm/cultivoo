...
// ─────────────────────────────────────────────
// CATEGORIAS E TIPOS — estrutura atualizada
// ─────────────────────────────────────────────

// Categorias disponíveis para lançamento MANUAL (pelo usuário)
// Cada entrada define: tipos, campos obrigatórios, e se é bloqueada (origemAutomatica)
export const CONFIG_CATEGORIAS = {
  // ── BLOQUEADAS — geradas automaticamente por outros módulos ──────────────
  'Insumos': {
  ...
  // Depreciação: categoria exclusiva, sem tipo (campo tipoDespesa = 'Depreciação' internamente)
  'Depreciação': {
    tipos: [],  // sem tipo para o usuário
    bloqueada: true,
    mensagem: 'A depreciação é calculada automaticamente com base no cadastro de patrimônios e não é lançada manualmente.',
    pagina: 'Patrimônio',
  },

  // ── PARCIALMENTE BLOQUEADAS — alguns tipos são automáticos ───────────────
  'Máquinas e Equipamentos': {
    // Removido Depreciação — agora tem categoria própria
  ...
  'Investimentos': {
    // Alinhado com categorias do Patrimônio
...
// Lista simplificada para dropdown de categoria (compatível com código legado)
...
// Adicionar meses a uma data ISO
...
function filtrarPorPeriodo(lista, filtro) {
  ...
  return lista.filter(l => {
    // Depreciação nunca aparece no financeiro
...
// ─────────────────────────────────────────────
// Patrimônio searchable (campo com busca)
// ─────────────────────────────────────────────
...
function PreviewParcelas({ dataInicial, numParcelas, valorTotal }) {
  ...
  const parcelas = Array.from({ length: Math.min(numParcelas, 60) }, (_, i) => ({
    num: i + 1,
    data: adicionarMeses(dataInicial, i),
...
export default function Financeiro() {
  ...
  // Área por propriedade (para rateio por área no Administrativo)
  const areasPorPropriedade = useMemo(() => {
    const m = {}
    lavouras.forEach(l => {
      if (l.propriedadeId) m[l.propriedadeId] = (m[l.propriedadeId] || 0) + (Number(l.areaHa) || 0)
    })
    return m
  }, [lavouras])

  // Safras filtradas pela propriedade selecionada no form
  ...
  const configCategoria = useMemo(() => {
    if (!form.categoria) return null
    return CONFIG_CATEGORIAS[form.categoria] || null
  }, [form.categoria])
  ...
  const tipoBloqueado = useMemo(() => {
    ...
    if (!cat?.tiposBloqueados) return null
    return cat.tiposBloqueados[form.tipoDespesa] || null
  }, [form.categoria, form.tipoDespesa])

  // Categoria completamente bloqueada?
  const categoriaBloqueada = useMemo(() => {
    if (!form.categoria) return null
    const cat = CONFIG_CATEGORIAS[form.categoria]
    if (!cat?.bloqueada) return null
    return cat
  }, [form.categoria])

  // Propriedades do patrimônio selecionado (para Máq. e Equip.)
  ...
  // Percentual de rateio para Administrativo
  function calcularPercentualRateioAdm(propId) {
    ...
    if (form.tipoRateio === 'personalizado') return Number(form.percentuaisRateio[propId]) || 0
    return 0
  }
  ...
  async function salvar(e) {
    ...
    if (form.tipo === 'despesa') {
      if (form.categoria === 'Administrativo' && form.propriedadeIds.length === 0)
        return alert('Selecione ao menos uma propriedade.')
      if (form.categoria === 'Administrativo' && form.tipoRateio === 'personalizado' && Math.abs(totalPercentuaisAdm() - 100) > 0.01)
        return alert('Os percentuais de rateio devem somar 100%.')
      if (form.categoria === 'Máquinas e Equipamentos' && !form.patrimonioId)
        return alert('Selecione o patrimônio vinculado.')
    }

    // Bloquear salvamento se categoria ou tipo for automático
    if (categoriaBloqueada || tipoBloqueado || (form.tipo === 'receita' && form.categoria === 'Receita Agrícola')) {
      setLoading(false)
      return
    }
    ...
    const payloadBase = {
      ...
      tipo: form.tipo,
      // Receitas Diversas salva como 'Receitas Diversas', não 'Receita Agrícola'
      categoria: form.tipo === 'receita'
        ? (form.categoria || 'Receitas Diversas')
        : form.categoria,
    ...
    if (form.tipo === 'despesa' && form.categoria === 'Administrativo') {
      // Lançamentos para múltiplas propriedades com rateio
      const ids = form.propriedadeIds
      const documentos = []
      ...
      for (const propId of ids) {
        ...
        for (let i = 0; i < parcelas; i++) {
          documentos.push({
            ...payloadBase,
            valor: parcelas > 1 ? valorProp / parcelas : valorProp,
            vencimento: adicionarMeses(dataVenc, i),
      ...
      if (editando) {
        await updateDoc(doc(db, 'financeiro', editando), documentos[0])
      } else {
        for (const d of documentos) await addDoc(collection(db, 'financeiro'), { ...d, criadoEm: new Date() })
      }

    } else if (form.tipo === 'despesa' && form.categoria === 'Máquinas e Equipamentos') {
      // Máquinas: usa patrimônio, propriedade vem do patrimônio (primeira)
      const pat = patrimonios.find(p => p.id === form.patrimonioId)
      ...
      for (let i = 0; i < parcelas; i++) {
        const payload = {
          ...payloadBase,
          valor: parcelas > 1 ? valorTotal / parcelas : valorTotal,
          vencimento: adicionarMeses(dataVenc, i),
          propriedadeId: propId,
      ...
      // Demais categorias (Cultivo, Investimentos, Receita manual)
  ...
  return (
    <div className="space-y-4 pb-24">
                  ...
                  return (
                    <button key={p.id} type="button"
                      ...
                      <span className={`w-3.5 h-3.5 rounded border flex-shrink-0 flex items-center justify-center ${selecionada ? 'bg-green-700 border-green-700' : 'border-gray-300'}`}>
                        {selecionada && <svg className="w-2.5 h-2.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}
                      </span>
                      {p.nome}
                    </button>
                  )
            ...
            <div key={propId} className="space-y-3">
              ...
              {meses.map(({ chave, itens }) => (
                <GrupoMes key={chave} chave={chave} itens={itens}
                  renderCard={l => <CardLancamento key={l.id} l={l} onEditar={abrirEdicao} onExcluir={excluir} />} />
              ))}
            </div>
          ))}
        </div>
        ...
        <div className="space-y-5">
          ...
          {agruparPorPropMes(contasPagar).map(({ propId, propNome, meses }) => (
            <div key={propId} className="space-y-3">
              ...
              {meses.map(({ chave, itens }) => (
                <GrupoMes key={chave} chave={chave} itens={itens}
                  renderCard={c => <CardConta key={c.id} c={c} tipoAcao="pagar"
        ...
        <div className="space-y-5">
          ...
          {agruparPorPropMes(contasReceber).map(({ propId, propNome, meses }) => (
            <div key={propId} className="space-y-3">
              ...
              {meses.map(({ chave, itens }) => (
                <GrupoMes key={chave} chave={chave} itens={itens}
                  renderCard={c => <CardConta key={c.id} c={c} tipoAcao="receber"
                    onEditar={abrirEdicao} onExcluir={excluir} onMarcarStatus={marcarStatus} />} />
              ))}
            </div>
          ))}
          ...
          <div className="flex flex-col items-end gap-2 mb-1">
            <div className="flex items-center gap-2">
              ...
              <button onClick={abrirImport}
                className="w-11 h-11 rounded-full bg-gray-500 text-white flex items-center justify-center shadow hover:bg-gray-600">
        ...
        <div data-modal className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl max-h-[92vh] overflow-y-auto">
            ...
            <form onSubmit={salvar} className="p-5 space-y-4">
              ...
              <div className="flex gap-3">
                {['despesa', 'receita'].map(t => (
                  <button key={t} type="button"
                    onClick={() => setForm(f => ({ ...f, tipo: t, categoria: '', tipoDespesa: '', status: '', safraId: '', propriedadeId: '', propriedadeIds: [], patrimonioId: '' }))}
              ...
              {form.tipo === 'despesa' && (
                <div>
                  ...
                  <select value={form.categoria}
                    onChange={e => setForm(f => ({ ...f, categoria: e.target.value, tipoDespesa: '', safraId: '', propriedadeId: '', propriedadeIds: [], patrimonioId: '' }))}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" required>
                    <option value="">Selecione...</option>
                    {Object.keys(CONFIG_CATEGORIAS).filter(k => !CONFIG_CATEGORIAS[k].ehReceita).map(c => (
              ...
              {categoriaBloqueada && (
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-amber-700 mt-0.5">{categoriaBloqueada.mensagem}</p>
                    <p className="text-xs text-amber-600 mt-1">📍 Página: <strong>{categoriaBloqueada.pagina}</strong></p>
                  </div>
                </div>
              )}

              {/* Tipo de despesa — só se categoria não estiver bloqueada */}
              {form.tipo === 'despesa' && form.categoria && !categoriaBloqueada && (
                <div>
                  ...
                  <select value={form.tipoDespesa}
                    ...
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" required>
                    <option value="">Selecione...</option>
                    {(CONFIG_CATEGORIAS[form.categoria]?.tipos || []).map(t => (
              ...
              {/* Card bloqueado — tipo específico automático */}
              {tipoBloqueado && (
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3">
                  <Lock size={16} className="text-amber-500 flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="text-sm font-medium text-amber-800">Lançamento automático</p>
                    <p className="text-xs text-amber-700 mt-0.5">{tipoBloqueado.mensagem}</p>
                    <p className="text-xs text-amber-600 mt-1">📍 Página: <strong>{tipoBloqueado.pagina}</strong></p>
                  </div>
                </div>
              )}

              {/* Campos condicionais — só exibir se não houver bloqueio e não for Receita Agrícola */}
              {!categoriaBloqueada && !tipoBloqueado && !(form.tipo === 'receita' && form.categoria === 'Receita Agrícola') && (
                <>
                  {/* Máquinas e Equipamentos — patrimônio searchable + propriedades travadas */}
                  {form.tipo === 'despesa' && form.categoria === 'Máquinas e Equipamentos' && (
                    <>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Patrimônio vinculado <span className="text-red-500">*</span></label>
                        <PatrimonioSearchable
                          value={form.patrimonioId}
                          onChange={id => setForm(f => ({ ...f, patrimonioId: id }))}
                          patrimonios={patrimonios}
                          obrigatorio
                        />
                      </div>
                      {propriedadesDoPatrimonio.length > 0 && (
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">Propriedade(s)</label>
                          <div className="bg-gray-50 rounded-lg px-3 py-2 space-y-1">
                            {propriedadesDoPatrimonio.map(p => (
                              <p key={p.id} className="text-sm text-gray-600">🏡 {p.nome}</p>
                            ))}
                            <p className="text-xs text-gray-400 mt-1">Vinculadas ao patrimônio selecionado (não editável)</p>
                          </div>
                        </div>
                      )}
                    </>
                  ...
                  {/* Administrativo — multiseleção com rateio */}
                  {form.tipo === 'despesa' && form.categoria === 'Administrativo' && (
                    <div>
                      ...
                      <div className="border border-gray-200 rounded-lg divide-y divide-gray-100 mb-2">
                        {propriedades.map(p => {
                          const sel = form.propriedadeIds.includes(p.id)
                          return (
                            <label key={p.id} className="flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-gray-50">
                              <span className={`w-4 h-4 rounded border flex-shrink-0 flex items-center justify-center ${sel ? 'bg-green-700 border-green-700' : 'border-gray-300'}`}
                                onClick={() => setForm(f => ({
                                  ...f,
                                  propriedadeIds: sel ? f.propriedadeIds.filter(id => id !== p.id) : [...f.propriedadeIds, p.id],
                                  percentuaisRateio: {},
                                }))}>
                      ...
                      {form.propriedadeIds.length > 1 && (
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">Tipo de rateio</label>
                          <div className="flex gap-2 mb-2">
                            {[{ val: 'igualitario', label: 'Igualitário' }, { val: 'area', label: 'Por área' }, { val: 'personalizado', label: 'Personalizado' }].map(op => (
                              <button key={op.val} type="button"
                                ...
                                className={`flex-1 py-1.5 text-xs rounded-lg border transition-colors ${form.tipoRateio === op.val ? 'border-green-600 bg-green-50 text-green-700 font-medium' : 'border-gray-200 text-gray-500'}`}>
                                {op.label}
                              </button>
                            ))}
                          </div>
                          <div className="space-y-1">
                            {form.propriedadeIds.map(id => {
                              ...
                              const pct = calcularPercentualRateioAdm(id)
                              return (
                                <div key={id} className="flex items-center justify-between gap-2 text-xs">
                                  ...
                                  {form.tipoRateio === 'personalizado' ? (
                                    <input type="number" min="0" max="100" step="0.1"
                                      ...
                                      className="w-20 border border-gray-200 rounded px-2 py-1 text-right text-xs focus:outline-none focus:ring-1 focus:ring-green-500"
                                      placeholder="%" />
                                  ) : (
                  ...
                  {/* Cultivo / Investimentos — propriedade única + safra opcional */}
                  {form.tipo === 'despesa' && (form.categoria === 'Cultivo' || form.categoria === 'Investimentos') && (
                    <>
                      <div>
                        ...
                        <select value={form.propriedadeId}
                          onChange={e => setForm(f => ({ ...f, propriedadeId: e.target.value, safraId: '' }))}
                          className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" required>
                          <option value="">Selecione...</option>
                          {propriedades.map(p => <option key={p.id} value={p.id}>{p.nome}</option>)}
                        </select>
                      </div>
                      {form.categoria === 'Cultivo' && form.propriedadeId && (
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">Safra <span className="text-gray-400 text-xs font-normal">(opcional)</span></label>
                          <select value={form.safraId}
                            ...
                            {safrasDoForm.map(s => <option key={s.id} value={s.id}>{s.nome}</option>)}
                          </select>
                        </div>
                      )}
                      {form.categoria === 'Investimentos' && (
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">Patrimônio vinculado <span className="text-gray-400 text-xs font-normal">(opcional)</span></label>
                          <PatrimonioSearchable
                            value={form.patrimonioId}
                            onChange={id => setForm(f => ({ ...f, patrimonioId: id }))}
                            patrimonios={patrimonios}
                  ...
                  {/* Receita manual legado (sem categoria selecionada — compatibilidade) */}
                  {form.tipo === 'receita' && !form.categoria && (
                    <>
                      <div>
                  ...
                  {!editando && (
                    <div>
                      <label className="flex items-center gap-3 cursor-pointer select-none">
                        <div className="relative flex-shrink-0" onClick={() => setForm(f => ({ ...f, parcelar: !f.parcelar }))}>
                          <div className={`w-10 h-6 rounded-full transition-colors ${form.parcelar ? 'bg-green-600' : 'bg-gray-300'}`}>
                            <div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${form.parcelar ? 'translate-x-5' : 'translate-x-1'}`} />
                          </div>
                        </div>
                      ...
                      {form.parcelar && (
                        <div className="mt-3 space-y-2">
                          <div className="flex items-center gap-3">
                            <label className="text-sm text-gray-600 whitespace-nowrap">Número de parcelas:</label>
                            <input type="number" min={2} max={60} value={form.numParcelas}
                              onChange={e => setForm(f => ({ ...f, numParcelas: Math.min(60, Math.max(2, Number(e.target.value))) }))}
                          ...
                          <PreviewParcelas
                            dataInicial={dataParaISO(form.vencimentoMask)}
                            numParcelas={form.numParcelas}
                            valorTotal={form.valorMask}
                          />
                        </div>
                      )}
                    </div>
                  ...
                  <div className="flex gap-3 pt-1">
                    ...
                    <button type="submit" disabled={loading}
                      ...
                      {loading ? 'Salvando...' : editando ? 'Atualizar' : form.parcelar ? `Gerar ${form.numParcelas} parcelas` : 'Salvar'}
                    </button>
                  </div>
                </>
              )}
              ...
              {(categoriaBloqueada || tipoBloqueado || (form.tipo === 'receita' && form.categoria === 'Receita Agrícola')) && (
                <button type="button" onClick={() => { setModal(false); setEditando(null) }}
                  className="w-full border border-gray-300 text-gray-600 py-2 rounded-lg text-sm hover:bg-gray-50">Fechar</button>
              )}
            </form>
          </div>
        </div>
      ...
      {/* Modal detalhes — lançamentos automáticos (ponto E) */}
      {modalDetalhe && (
        <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-gray-800">Detalhes do lançamento</h3>
              <button onClick={() => setModalDetalhe(null)} className="text-gray-400 hover:text-gray-600"><X size={18} /></button>
            </div>
            <div className="bg-gray-50 rounded-xl p-3 space-y-2">
              {[
                { label: 'Descrição', valor: modalDetalhe.descricao },
                { label: 'Tipo', valor: modalDetalhe.tipo === 'receita' ? 'Receita' : 'Despesa' },
                { label: 'Categoria', valor: modalDetalhe.categoria },
                { label: 'Tipo detalhe', valor: modalDetalhe.tipoDespesa || '—' },
                { label: 'Valor', valor: `R$ ${formatarMoeda(modalDetalhe.valor)}` },
                { label: 'Data', valor: formatarDataBR(modalDetalhe.vencimento) },
                { label: 'Situação', valor: modalDetalhe.status || '—' },
                { label: 'Propriedade', valor: modalDetalhe.propriedadeNome || '—' },
                { label: 'Safra', valor: modalDetalhe.safraNome || '—' },
                { label: 'Patrimônio', valor: modalDetalhe.patrimonioNome || '—' },
                { label: 'Nº Doc.', valor: modalDetalhe.notaRef || '—' },
              ].filter(r => r.valor && r.valor !== '—').map(row => (
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
              <p className="text-xs text-amber-700">
                🔒 Este lançamento foi gerado automaticamente e não pode ser editado. Para correções, acesse a página de origem.
              </p>
            </div>
            <button onClick={() => setModalDetalhe(null)}
              className="w-full border border-gray-300 text-gray-600 py-2 rounded-xl text-sm hover:bg-gray-50">Fechar</button>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl">
            ...
            <div className="p-5 space-y-4">
              ...
              {previewImport.length > 0 && (
                <div className="space-y-1">
                  <p className="text-xs font-semibold text-gray-500">Prévia (5 primeiras linhas):</p>
                  {previewImport.map((r, i) => (
                    <div key={i} className="text-xs text-gray-600 bg-gray-50 rounded px-2 py-1 truncate">{r.join(' | ')}</div>
                  ))}
                </div>
              )}
              <div className="flex gap-3">
                <button onClick={() => setModalImport(false)} className="flex-1 border border-gray-300 text-gray-600 py-2 rounded-lg text-sm hover:bg-gray-50">Cancelar</button>
